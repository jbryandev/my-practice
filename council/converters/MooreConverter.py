...
class MooreConverter(PDFConverter):
    ...
    def format_text(self, pdf_text):
        ...
        remove_action = re.sub(r"ACTION:(\sa)*", "", remove_page)
    ...
    def indent_text(self, trimmed_text):
        if re.match(r"\d{1,2}\)\s[A-Z][A-Za-z]{2,}", trimmed_text):
            ...
            end = re.search(r"(\d{1,2}|[A-Z])\)\s[A-Z][A-Za-z]{2,}|\n[A-Z]{3,}", trimmed_text[start.end():])
            if end:
        ...
        elif re.match(r"[A-Z]\)\s[A-Z][A-Za-z]{2,}", trimmed_text):
            start = re.match(r"[A-Z]\)\s[A-Z][A-Za-z]{2,}", trimmed_text)
            end = re.search(r"(\d{1,2}|[A-Z])\)\s[A-Z][A-Za-z]{2,}|\n[A-Z]{3,}", trimmed_text[start.end():])
            if end:
